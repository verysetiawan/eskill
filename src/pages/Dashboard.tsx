                                 ...
                                 <div className="bg-white shadow-2xl relative overflow-hidden font-sans text-black w-full max-w-[620px] aspect-[1/1.414] shrink-0 px-[7%] pt-[6%] pb-[4.5%] leading-tight">
                                   ...
                                   <div className="grid grid-cols-2 gap-x-16 gap-y-3 mt-2 text-[8px] font-bold">
                                     <div className="text-center">
                                       ...
                                       <div className="h-11 flex items-start justify-center">
                                         {(isLSPPreview ? settings.school.certificationManagerSignature : department?.competencyHeadSignature) && (
                                           <img src={isLSPPreview ? settings.school.certificationManagerSignature : department?.competencyHeadSignature} className="max-h-11 max-w-[124px] object-contain" />
                                         )}
                                       </div>
                                     ...
                                     <div className="text-center">
                                       ...
                                       <div className="h-11 flex items-start justify-center">
                                         {examiner?.signature && <img src={examiner.signature} className="max-h-11 max-w-[124px] object-contain" />}
                                       </div>
                                     ...
                                     <div className="text-center pt-5">
                                       ...
                                       <p>Kepala Sekolah</p>
                                       <div className="h-11 flex items-start justify-center">
                                         {settings.school.signatorySignature && <img src={settings.school.signatorySignature} className="max-h-11 max-w-[124px] object-contain" />}
                                       </div>
                                     ...
                                     <div className="text-center pt-5">
                                       ...
                                       <div className="h-11 flex items-start justify-center">
                                         {(isLSPPreview ? settings.school.lspDirectorSignature : settings.school.vicePrincipalCurriculumSignature) && (
                                           <img src={isLSPPreview ? settings.school.lspDirectorSignature : settings.school.vicePrincipalCurriculumSignature} className="max-h-11 max-w-[124px] object-contain" />
                                         )}
                                       </div>
                      ...
                      <div className="space-y-3 md:col-span-2">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          ...
                          <div className="flex gap-2">
                            <button
                              type="button"
                              onClick={() => setSelectedDeleteClasses(availableClasses)}
                              disabled={availableClasses.length === 0}
                              className="px-3 py-2 rounded-xl border border-red-100 bg-white text-[10px] font-bold text-red-600 uppercase tracking-wider hover:bg-red-50 disabled:opacity-50 transition-colors"
                            >
                              Pilih Semua
                            </button>
                            <button
                              type="button"
                              onClick={() => setSelectedDeleteClasses([])}
                              className="px-3 py-2 rounded-xl border border-border bg-white text-[10px] font-bold text-slate-500 uppercase tracking-wider hover:bg-slate-50 transition-colors"
                            >
                              Semua Kelas
                            </button>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-44 overflow-y-auto bg-white border border-border rounded-2xl p-3 shadow-sm">
                          {availableClasses.length > 0 ? availableClasses.map(className => {
                            const checked = selectedDeleteClasses.includes(className);
                            return (
                              <label
                                key={className}
                                className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold cursor-pointer transition-all ${
                                  checked
                                    ? 'border-red-200 bg-red-50 text-red-700'
                                    : 'border-slate-100 bg-white text-primary hover:border-red-100 hover:bg-red-50/40'
                                }`}
                              >
                                <input
                                  type="checkbox"
                                  checked={checked}
                                  onChange={(e) => {
                                    setSelectedDeleteClasses(prev => e.target.checked
                        ...
                        <p className="text-[10px] text-text-muted font-medium ml-1">
                          Tidak memilih kelas berarti semua kelas pada filter jurusan/tahun akan dihapus.
                        </p>
                      </div>
                   </div>
                   ...
                   <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-6 p-6 bg-red-50/50 rounded-2xl border border-red-100">
                      <div className="flex items-center gap-4">
                         <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold text-lg">
                            {students.filter(s => {
                               const matchesDept = selectedDepartment === 'Semua Jurusan' || s.Jurusan === selectedDepartment;
                               const matchesYear = selectedYear === 'Semua Tahun' || s['Tahun Lulus'] === selectedYear;
                               const matchesClass = selectedDeleteClasses.length === 0 || selectedDeleteClasses.includes(s.Kelas);
                               return matchesDept && matchesYear && matchesClass;
                            }).length}
                         </div>
                         <div>
                      ...
                      <button 
                        ...
                        onClick={async () => {
                           ...
                           const message = selectedDepartment === 'Semua Jurusan' && selectedYear === 'Semua Tahun' && selectedDeleteClasses.length === 0
